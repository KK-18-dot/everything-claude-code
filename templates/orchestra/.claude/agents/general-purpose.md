...
**CRITICAL**: Background subagents cannot answer permission prompts. If a tool call is not already auto-approved, it will be auto-denied.

- Use `run_in_background: true` only for summary-only work or CLI calls that are already allowed.
- Use `run_in_background: false` when the task must create/update files, needs shell redirection, or may require a new permission grant.
- Do **not** use shell redirection (`>`, `>>`, `2>&1 > file`), here-docs, or filesystem MCP writes inside background Gemini tasks.
- For Gemini research in a subagent:
  1. Run `gemini -p "..." 2>/dev/null` with no output redirection.
  2. Capture stdout in the Bash result.
  3. Save the result with the `Write` tool if persistence is required.
  4. If file persistence may need approval, return the summary and let the main agent save it.
- Prefer existing directories such as `.claude/docs/research/`. Do not create directories from a background subagent unless the command is explicitly allowlisted.
...
1. Call Gemini CLI for research with no shell redirection
2. Summarize key findings (5-7 bullet points)
3. Save detailed output with Write if allowed, otherwise return summary only
